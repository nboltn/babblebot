...
fn main() {
    ...
    if let Some(matches) = matches.subcommand_matches("add_channel") { add_channel(con.clone(), &settings, matches) }
        ...
        loop {
            let bots: HashSet<String> = con.smembers("bots").unwrap();
            for bot in bots {
                let passphrase: String = con.get(format!("bot:{}:token", bot)).unwrap();
                let channel_hash: HashSet<String> = conn.smembers(format!("bot:{}:channels", bot)).unwrap();
                ...
                let client_handler = reactor.prepare_client_and_connect(&config).unwrap();
                let client_timers = Arc::new(reactor.prepare_client_and_connect(&config).unwrap());
                client_handler.identify().unwrap();
                client_timers.identify().unwrap();
                let _ = client_handler.send("CAP REQ :twitch.tv/tags");
                let _ = client_handler.send("CAP REQ :twitch.tv/commands");
                register_handler(client_handler, &mut reactor, con.clone());
                for channel in channel_hash.iter() {
                    live_update(client_timers.clone(), pool.clone(), channel.to_owned());
                }
            }
            let res = reactor.run();
            match res {
...
fn live_update(client: Arc<IrcClient>, pool: r2d2::Pool<r2d2_redis::RedisConnectionManager>, channel: String) {
    thread::spawn(move || {
        let con = Arc::new(pool.get().unwrap());
                        ...
                        Ok(json) => {
                            ...
                            if json.total == 0 {
                                ...
                                if live == "false" {
                                    ...
                                    spawn_timers(client.clone(), pool.clone(), channel.to_owned());
                                }
                            }
                        }
                    }
...
fn spawn_timers(client: Arc<IrcClient>, pool: r2d2::Pool<r2d2_redis::RedisConnectionManager>, channel: String) {
    thread::spawn(move || {
...
fn add_channel(con: Arc<r2d2::PooledConnection<r2d2_redis::RedisConnectionManager>>, settings: &config::Config, matches: &ArgMatches) {
    ...
    match matches.value_of("botToken") {
        Some(token) => { bot_token = token.to_string(); },
